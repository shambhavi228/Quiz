...
function showQuestionM(){
  resetState();
   let currentQuestion=coding[currentQuestionIndex];
   let questionNo=currentQuestionIndex+1;
  // let questionNo=coding.id;
  questionElement.innerHTML=questionNo+". "+currentQuestion.question;
  currentQuestion.options.forEach(answer =>{
      const button=document.createElement("button");
      const ans=coding.answer;
       if(answer==ans){
...
function handleNextButton(){
  ...
  if(currentQuestionIndex<coding.length){
      showQuestionM();
  }
  else{
      showScore();
...
function showScore(){
  resetState();
  questionElement.innerHTML=`Your Score is ${score} out of ${coding.length}!`;
